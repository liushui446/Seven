...
# 分离两个管道句柄（纯同步模式）
hPipe_send = None  # 发送命令用：ClientToServerPipe
hPipe_listen = None  # 监听结果用：ServerToClientPipe
...
# 管道名称常量（与服务端对应）
PIPE_NAME_CLIENT_TO_SERVER = r"\\.\pipe\ClientToServerPipe"  # 发送命令
PIPE_NAME_SERVER_TO_CLIENT = r"\\.\pipe\ServerToClientPipe"  # 监听结果
...
def send_pipe_command_raw(cmd: dict):
    """发送命令并返回解析后的 JSON 响应（None 表示失败）"""
    global hPipe_send
    if hPipe_send is None:
        print("[实时] 发送管道未初始化")
        return None
    try:
        cmd_str = json.dumps(cmd, ensure_ascii=False) + "\n###END###\n"
        cmd_bytes = cmd_str.encode("utf-8")
        win32file.WriteFile(hPipe_send, cmd_bytes)
        read_result = win32file.ReadFile(hPipe_send, 4095 * 10000)
        result_bytes = bytes(read_result[1])
...
def realtime_loop():
    ...
    while is_realtime_running and is_listening:
        ...
        # --- 发送 & 接收 ---
        result = send_pipe_command_raw(cmd)
        if result is None:
            print("[实时] 通信失败，重试...")
            time.sleep(0.1)
            continue
...
def send_pipe_command(cmd: dict) -> bool:
    """纯同步发送：仅操作发送管道，同步写入+同步读取结果"""
    global hPipe_send
    if hPipe_send is None:
        print("[发送] 发送管道句柄未初始化，发送失败")
        return False
    ...
    try:
        ...
        # 2. 同步写入（纯同步模式）
        bytes_written = win32file.WriteFile(hPipe_send, cmd_bytes)[0]
        print(f"[发送] 发送字节数：{len(cmd_bytes)}，实际写入：{bytes_written}")

        # 3. 同步读取计算结果（纯同步，适配memoryview类型）
        buffer_size = 4095 * 10000
        # 同步读取：返回 (错误码, memoryview对象)
        read_result = win32file.ReadFile(hPipe_send, buffer_size)
        result_mv = read_result[1]  # 获取memoryview对象
        # 核心修复：将memoryview转为bytes
        ...
        # 4. 解析结果（修复decode错误）
        result_str = result_bytes.decode("utf-8", errors="ignore")  # 容错解码
        ...
        error_code = e.args[0]
        print(f"[发送] 管道异常，错误码：{error_code}，信息：{e.args[2]}")
        
        # 句柄失效重连
        if error_code in [109, 6]:  # 管道断开/句柄无效
            print("[发送] 发送管道句柄失效，尝试重连...")
...
def listen_pipe_continually():
    """纯同步监听：解析编队仿真数据并缓存到 formation_data 供可视化使用"""
    global is_listening, hPipe_listen, formation_data, max_cached_frame
    if hPipe_listen is None:
        print("[监听] 监听管道句柄未初始化，退出")
        return
    ...
    while is_listening:
        try:
            buffer_size = 4095 * 10000
            read_result = win32file.ReadFile(hPipe_listen, buffer_size)
            result_mv = read_result[1]
            result_bytes = bytes(result_mv)
            bytes_read = len(result_bytes)
            ...
            if err == 109:
                ...
                print(f"[监听] 管道异常: 错误码={err}, "
                      f"信息={e.args[2] if len(e.args) > 2 else '未知'}")
                time.sleep(0.01)
                continue
...
def create_pipe(pipe_name: str, is_listen_pipe: bool = False) -> pywintypes.HANDLE or None:
    """创建纯同步管道句柄（移除所有异步相关配置）"""
    # 同步模式权限配置
    if is_listen_pipe:
        # 监听管道：仅读
    ...
    for i in range(5):
        try:
            ...
            error_code = e.args[0]
            if error_code == 231:  # 管道忙
                print(f"[初始化] 管道忙 {pipe_name}（{i+1}/5），等待1秒...")
                time.sleep(1)
            elif error_code == 5:  # 权限拒绝
                print(f"[初始化] 管道权限拒绝 {pipe_name}（{i+1}/5），检查服务端配置...")
                time.sleep(1)
            else:
                print(f"[初始化] 连接失败 {pipe_name}（{i+1}/5）：{e.args[2]}（错误码：{error_code}）")
                time.sleep(1)

    # 重试5次仍失败
    if pipe_handle is None or pipe_handle == win32file.INVALID_HANDLE_VALUE:
        print(f"[初始化] 管道创建失败 {pipe_name}，重试5次后退出")
        return None
    ...
    try:
        ...
        print(f"[初始化] 设置管道模式失败 {pipe_name}：{e.args[2]}")
        win32file.CloseHandle(pipe_handle)
        return None
    
    print(f"[初始化] 管道句柄创建成功：{pipe_name}（{i+1}/5）")
    return pipe_handle

def init_both_pipes() -> bool:
    """初始化两个纯同步管道"""
    global hPipe_send, hPipe_listen
    
    # 1. 创建发送管道（双向）
    hPipe_send = create_pipe(PIPE_NAME_CLIENT_TO_SERVER, is_listen_pipe=False)
    if hPipe_send is None:
        print("[初始化] 发送管道创建失败")
        return False
    
    # 2. 创建监听管道（双向）
    hPipe_listen = create_pipe(PIPE_NAME_SERVER_TO_CLIENT, is_listen_pipe=False)
    if hPipe_listen is None:
        print("[初始化] 监听管道创建失败")
        # 关闭已创建的发送管道
        if hPipe_send is not None:
            win32file.CloseHandle(hPipe_send)
            hPipe_send = None
        return False
...
def reconnect_send_pipe() -> bool:
    ...
    new_handle = create_pipe(PIPE_NAME_CLIENT_TO_SERVER, is_listen_pipe=False)
    if new_handle is not None:
        hPipe_send = new_handle
        return True
    return False
...
def reconnect_listen_pipe() -> bool:
    """重连监听管道"""
    global hPipe_listen
    if hPipe_listen is not None:
        try:
            win32file.CloseHandle(hPipe_listen)
        except:
            pass
        hPipe_listen = None

    # 监听管道必须传 True
    new_handle = create_pipe(PIPE_NAME_SERVER_TO_CLIENT, is_listen_pipe=True)
    if new_handle is not None:
        hPipe_listen = new_handle
        return True
    return False
...
def send_calc_command(cmd: dict) -> dict:
    ...
    send_success = send_pipe_command(cmd)
    if not send_success:
...
def main():
    ...
    # 初始化两个纯同步管道
    print("[初始化] 开始创建双管道连接（纯同步模式）...")
    if not init_both_pipes():
        print("[初始化] 双管道初始化失败，退出")
        return
    ...
    try:
        while True:
            ...
            if user_input.lower().startswith('rformup '):
                ...
                if spd > 0:
                    print(f"[实时] 编队{fid} 靠拢模式开启 speed={spd}m/s (靠近<30m线性减速到主船航速)")
                else:
            ...
            if user_input.lower() == 'status':
                ...
                print(f"发送管道: {'已连接' if hPipe_send else '未连接'}")
                print(f"监听管道: {'已连接' if hPipe_listen else '未连接'}")
