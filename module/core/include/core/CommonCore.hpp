...
// ===================== 跨平台 JSON 头文件（必须在标准头之后）=====================
#ifdef _WIN32
    // Windows: 使用 3rdparty 中的 jsoncpp
    #include "json.h"
#else
    // Linux/麒麟: 使用系统安装的 jsoncpp
...
// ===================== 跨平台类型定义 =====================
// Windows 类型兼容（Linux 下模拟 Windows 基本类型）
// 用 #ifndef 保护，避免与 <windows.h> 的重复 typedef 冲突（MSVC 14.39+ 会报错）
#ifndef DWORD
...
// Windows 句柄类型兼容
#ifdef _WIN32
    // Windows: HANDLE 由 <windows.h> 提供，此处不重复定义
    // INVALID_HANDLE_VALUE 由 <windows.h> 提供
#else
    // Linux: 用 int 模拟 HANDLE
    typedef int HANDLE;
...
namespace seven
{
    ...
    */
    ...
    using FormationMap = std::map<int, FormationConfig>;
